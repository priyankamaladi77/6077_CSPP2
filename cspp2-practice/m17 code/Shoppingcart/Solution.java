    ...
    public void addToCart(final Item item)  {
        ...
        if (index == -1) {
            for (int i = 0; i < catasize; i++) {
                ...
                && (cata[i].getQuantity() >= item.getQuantity())) {
                    cart[cartsize] = item;
                    cartsize += 1;
                }
            }
        } else {
            int quan = cart[index].getQuantity()
            + item.getQuantity();
    ...
    public double getTotalAmount() {
        ...
        for (int i = 0; i < cartsize; i++) {
            for (int j = 0; j < catasize; j++) {
                if (cart[i].getProductName().
                    equals(cata[j].getProductName())) {
                    amu += cart[i].getQuantity()
                    * cata[j].getUnitPrice();
                }
            }
        }
    ...
    public void isCoupon(final String coupon) {
        for (int i = 0; i < coupons.length; i++) {
            if (coupon.equals(coupons[i]) && !couponavail) {
                coup = Character.
                getNumericValue(coupons[i].charAt(THREE));
                couponavail = true;
                break;
            }
        }
        ...
        while (sc.hasNext()) {
            ...
            switch (tokens[0]) {
                ...
                cart.addToCatalog(new Item(items[0],
                    Integer.parseInt(items[1]), Double.parseDouble(items[2])));
                break;
                case "add":
